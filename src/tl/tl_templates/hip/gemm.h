...
class GemmTensorOp {
 public:
  ...
  CK_TILE_DEVICE static constexpr auto linear_index_map(int thread_id, int local_id) {
    return std::make_pair(thread_id / 4, (thread_id % 4) * 4 + local_id);
  }
  ...
  static CK_TILE_DEVICE void body(A_type* A_shared, B_type* B_shared, C_type* C_local) {
    ...
    for (int ki = 0; ki < inner_k; ki++) {
      ...
      for (int i = 0; i < warp_rows; i++) {
        ...
        ((float16x4*)A_local)[i] =
            ((float16x4*)A_shared)[(lane_id * local_size_a + ki * (warp_size * local_size_a) +
                                    (tz * warp_rows + i) * (inner_k * warp_size * local_size_a)) /
                                   local_size_a];
      }
      ...
      //       printf("A_local tid %d %f \n", static_cast<int>(tid), (float)A_local[i * local_size_a +
      //       j]);
      //     }
      //   }
      // }
      ...
      for (int j = 0; j < warp_cols; j++) {
        for (int local_id = 0; local_id < local_size_b; local_id++) {
          ...
          ((float16x4*)B_local)[j] =
              ((float16x4*)
                   B_shared)[(local_id + lane_id * local_size_b + ki * (warp_size * local_size_b) +
                              (ty * warp_cols + j) * (inner_k * warp_size * local_size_b)) /
                             local_size_b];
        }
      }
      ...
      // if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
      //   // print A_local
      //   for (int i = 0; i < warp_cols; i++) {
      //     for (int j = 0; j < local_size_b; j++) {
      //       printf("B_local tid %d %f \n", static_cast<int>(tid),
      //              (float)B_local[i * local_size_b + j]);
      //     }
      //   }
      // }
