...
void BlockAttrs(ffi::Map<ffi::String, Any> attrs) {
  ...
  if (sblock_frame.defined()) {
    if (!sblock_frame.value()->annotations.defined()) {
      ...
      sblock_frame.value()->annotations = Downcast<ffi::Map<ffi::String, Any>>(
          MergeAnnotations(Downcast<ffi::Map<Any, Any>>(attrs),
                           Downcast<ffi::Map<Any, Any>>(
                               sblock_frame.value()->annotations.value())));
    }
    return;
  }
  TVM_FFI_THROW(InternalError)
    ...
    .set_dispatch<tvm::tirx::BufferNode>([](const ffi::ObjectRef& node, ffi::String name) -> void {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        if (auto v = e.as<tvm::tirx::Var>()) {
          ffi::String new_name = !v.value()->name_hint.empty() ? v.value()->name_hint : (name + "_s" + std::to_string(i));
          Namer::Name(v.value(), ffi::String(new_name));
        }
      }
    });
