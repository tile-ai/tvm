...
Type GetType(const PrimExpr& expr) {
  ...
  if (auto* call = expr.as<tir::CallNode>()) {
    if (call->op.same_as(builtin::tvm_access_ptr())) {
      ICHECK(call->args.size()) << "Builtin tvm_access_ptr() may not have empty arguments";
      auto type_annotation = Downcast<Call>(call->args[0]);
    ...
    if (call->op.same_as(builtin::address_of())) {
      ICHECK_EQ(call->args.size(), 1)
          ...
          << call->args;
      auto* address = call->args[0].as<BufferLoadNode>();
      ICHECK(address)
...
void BinaryOpMatchTypes(PrimExpr& lhs, PrimExpr& rhs, const Span& span) {
  LOG(INFO) << "BinaryOpMatchTypes - Initial types:";
  LOG(INFO) << "  lhs type: " << lhs.dtype();
  LOG(INFO) << "  rhs type: " << rhs.dtype();

  if (lhs.dtype() == rhs.dtype()) {
    LOG(INFO) << "Types match, no conversion needed";
    return;
  }
  ...
  LOG(INFO) << "Attempting type conversion:";
  LOG(INFO) << "  Target type: " << dtype;
  LOG(INFO) << "  Source type: " << rhs_dtype;

  // Handle float8 types specially
  if (dtype.is_float8() || rhs_dtype.is_float8()) {
    LOG(INFO) << "Handling float8 type conversion";
    // Handle vector types
    if (dtype.lanes() > 1 || rhs_dtype.lanes() > 1) {
      if (dtype.lanes() != rhs_dtype.lanes()) {
        if (dtype.lanes() > 1) {
          rhs = tir::Broadcast(rhs, dtype.lanes());
        } else {
          lhs = tir::Broadcast(lhs, rhs_dtype.lanes());
        }
      }
      // Convert element type while preserving vector lanes
      if (dtype.is_float8()) {
        if (rhs_dtype.is_float()) {
          rhs = tvm::cast(dtype, rhs);
        } else if (rhs_dtype.is_int() || rhs_dtype.is_uint()) {
          rhs = tvm::cast(dtype, tvm::cast(DataType::Float(32), rhs));
        }
      } else {
        if (dtype.is_float()) {
          lhs = tvm::cast(rhs_dtype, lhs);
        } else if (dtype.is_int() || dtype.is_uint()) {
          lhs = tvm::cast(rhs_dtype, tvm::cast(DataType::Float(32), lhs));
        }
      }
      return;
    }
    // Handle scalar types
    if (dtype.is_float8()) {
      LOG(INFO) << "Converting rhs to float8";
      if (rhs_dtype.is_float()) {
        rhs = tvm::cast(dtype, rhs);
        return;
      }
      if (rhs_dtype.is_int() || rhs_dtype.is_uint()) {
        rhs = tvm::cast(dtype, tvm::cast(DataType::Float(32), rhs));
        return;
      }
    } else {
      LOG(INFO) << "Converting lhs to match rhs";
      if (dtype.is_float()) {
        lhs = tvm::cast(rhs_dtype, lhs);
        return;
      }
  ...
  if (dtype.lanes() != rhs_dtype.lanes()) {
    LOG(INFO) << "Lane mismatch: " << dtype.lanes() << " vs " << rhs_dtype.lanes();
    if (dtype.lanes() > 1) {
      rhs = tir::Broadcast(rhs, dtype.lanes());
    } else {
      lhs = tir::Broadcast(lhs, rhs_dtype.lanes());
  ...
  if (dtype.is_float()) {
    LOG(INFO) << "Converting to float type";
    rhs = tvm::cast(dtype, rhs);
  } else if (dtype.is_int()) {
    LOG(INFO) << "Converting to int type";
    rhs = tvm::cast(dtype, rhs);
  } else if (dtype.is_uint()) {
    LOG(INFO) << "Converting to uint type";
    rhs = tvm::cast(dtype, rhs);
  }
}
