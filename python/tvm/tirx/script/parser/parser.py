...
from tvm.tirx import Buffer, BufferLoad, IterVar, Layout, PrimExpr, Var
...
def visit_assign(self: Parser, node: doc.Assign) -> None:
    ...
    if isinstance(lhs_value, T.scalar_wrapper | BufferLoad | tvm.tirx.Buffer):
        if isinstance(lhs_value, T.scalar_wrapper):
            buffer = lhs_value.scalar.buffer
        else:
            buffer = lhs_value.buffer if isinstance(lhs_value, BufferLoad) else lhs_value
